# SKILL: repository_native_operator_learning

This is a redirect to the canonical skill implementation.

**Canonical implementation:** See [`tools/skills/repository_native_operator_learning/SKILL.md`](../../../tools/skills/repository_native_operator_learning/SKILL.md)

Key boundary: RNOLA is advisory Zone 1 learning/calibration only. It does not merge, ratify, sign, deploy, grant permissions, or treat an operator-check record as authorization.

For theory and governance context, see [`docs/INTENT_OS_RNOLA_INTEGRATION.md`](../../../docs/INTENT_OS_RNOLA_INTEGRATION.md).
