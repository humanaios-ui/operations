...
Any REGISTERED.md append entry produced by the MOLT phase's outcome-processing path — specifically entries triggered by a node status change based on `task_outcomes` evidence (the Task Outcome Recorder feedback loop, Gap A closure) rather than by initial LI-threshold assessment alone — requires explicit Zone 2 ratification before Night merge. The producing tool shall mark such entries `ratification_required: true` to distinguish them from standard LI-based promotions (`ratification_required: false`). This gate exists because outcome-driven status changes carry a different evidence quality than initial ACAT-scored promotions: they reflect execution consequences, not fresh independent assessment. Outcome-driven entries must not bypass the Zone 2 finding registration gate (P21) by virtue of automated origin. This ratification unblocks Gap A Task 6 (wiring `task_outcomes` into `run_molt_cycle()`). Violation: merging an outcome-driven REGISTERED.md entry without Zone 2 ratification, or generating such an entry without the `ratification_required` field correctly set.
