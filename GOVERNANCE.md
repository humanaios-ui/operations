...
**P30 — Calibration Ratification Gate** *(ratified Night · S-061726-01 · June 17, 2026)*
Before any substantive written artifact is presented for Zone 2 ratification, the producing substrate must run it through `acat_document_analyzer_v1.1` in interactive mode — evidence-density anchored, analyst-reviewed scores — and attach the resulting report as a `calibration_ref` to the ratification request.

Scope: papers, specs, skills, position-paper outlines, and governance documents are in scope. Atomic registry entries (F/H/IC filings) and code changes without an accompanying reference document are out of scope — there is nothing for a 12-dimension document instrument to meaningfully score.

Rigor requirement: the automated batch / evidence-density-only pass (`--batch-dir`) is acceptable only for early or explicitly-internal drafts not yet headed for Z2. Anything actually presented for ratification requires the interactive pass. A `calibration_ref` generated by the batch pass does not satisfy this gate.

This calibration analysis is a structured self-report, not independent verification — the same Phase 1 / Phase 3 distinction the instrument applies everywhere else. Night’s Z2 review remains the actual check; the `calibration_ref` gives that review something concrete to evaluate, not a replacement for it, consistent with the calibration circularity guard already established in P29.

No new infrastructure required. `document_ingestor_v1_0.py` and the `document_layer` field on `acat_assessments_v1` already support this — calibration analyses stage under the existing `governance_document` layer with `corpus_eligible: pending_Z2`. Running the gate is mandatory; corpus inclusion remains the separate Z2 decision it already was.

Violation: an artifact presented for Z2 ratification without an attached `calibration_ref`, or with one generated by the batch pass instead of the required interactive pass, has not cleared the gate. Ratification does not proceed until the correct-rigor calibration analysis is attached.

**P31 — Outcome-Driven Registry Gate** *(ratified Night · S-062426-01 · June 24, 2026)*
Any REGISTERED.md append entry produced by the MOLT phase's outcome-processing path — specifically entries triggered by a node status change based on `task_outcomes` evidence (the Task Outcome Recorder feedback loop, Gap A closure) rather than by initial LI-threshold assessment alone — requires explicit Zone 2 ratification before Night merges. The producing tool shall mark such entries `ratification_required: true` to distinguish them from standard LI-based promotions (`ratification_required: false`). This gate exists because outcome-driven status changes carry a different evidence quality than initial ACAT-scored promotions: they reflect execution consequences, not fresh independent assessment. Outcome-driven entries must not bypass the Zone 2 finding registration gate (P21) by virtue of automated origin. This ratification unblocks Gap A Task 6 (wiring `task_outcomes` into `run_molt_cycle()`). Violation: merging an outcome-driven REGISTERED.md entry without Zone 2 ratification, or generating such an entry without the `ratification_required` field correctly set.
...
- **v6.4.3 (June 24, 2026 · S-062426-01):** P31 (Outcome-Driven Registry Gate) ratified Night · S-062426-01. Requires Zone 2 ratification before Night merges any REGISTERED.md append entry produced by the MOLT phase's outcome-processing path (`task_outcomes` feedback loop). Entries produced via initial LI-threshold assessment are unaffected. This ratification unblocks Gap A Task 6 (wiring `task_outcomes` into `run_molt_cycle()` in `haios_agent_orchestrator_v1_0_patched.py`). Total principle count: 29 ratified integer principles + 1 ratified decimal (P22.1) + 2 honest gaps (P9, P14) = 32 P-numbers in registry.
- **v6.4.2 (June 17, 2026 · S-061726-01):** P30 (Calibration Ratification Gate) ratified Night · S-061726-01. Requires the interactive evidence-density-anchored ACAT document analysis pass (not the automated batch pass) on any substantive written artifact — papers, specs, skills, position papers, governance documents — before Zone 2 ratification, attached as `calibration_ref`. Self-report framing tied to the calibration circularity guard already established in P29. No new infrastructure; uses existing `document_ingestor_v1_0.py` and `document_layer` field. Total principle count: 29 ratified principles + 2 honest gaps (P9, P14) + P30 = 31 P-numbers in registry.
