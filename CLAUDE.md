...
1. Read `REGISTERED.md` at that SHA. Compare against the last pinned blob in `z1-inbox/*/ic030_live_read_*.md`. Report drift as IC-CAND; never smooth it.
1. Read the newest `z1-inbox/<date>/MANIFEST.md`. Verify each file’s sha256. A mismatch is a loud failure, not a warning.
1. Read `PRIORITY_QUEUE.md` (or the queue patch in the inbox). Only READY top-score rows begin; blocked rows get their unblock action only.
1. State position · destination · probability. Then work.
