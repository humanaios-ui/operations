...
def _evaluate_build_success(
    ...
    found_services = {svc for svc in required_services if any(svc in oid.lower() for oid in build_outcomes.keys())}
    if found_services != required_services:
        return ClaimEvaluation(
            ...
            stated_confidence=claim.stated_confidence,
            verification_status=VerificationStatus.UNKNOWN,
            actual_outcome=f"Build outcomes incomplete; found {found_services}, need {required_services}",
            calibration_score=None,
            evidence="Nix/Railpack build logs not captured for all required services",
            z2_gate="ADVISORY",
        )
...
def _evaluate_deployment_time(
    ...
    # Claimed: 5–10 minutes (300-600 seconds)
    ...
    # Claim window: 5-10 minutes = 300-600 seconds
    min_seconds = 300
    max_seconds = 600

    within_tolerance = min_seconds <= actual_seconds <= max_seconds
...
def get_pr465_claims() -> list[Claim]:
    ...
    NOTE: This is a hard-coded fixture. For production, this should parse the actual
    PR body from GitHub to extract falsifiable claims dynamically.
    """
    return [
        Claim(
            claim_id="PR465-C1",
...
def render_markdown_report(report: ComparisonReport) -> str:
    ...
    calibration_str = f"{report.calibration_metric:.2f}" if report.calibration_metric is not None else "N/A"
    ...
    lines = [
        ...
        f"| Calibration (MAE) | {calibration_str} |",
    ...
    for claim in report.claims:
        ...
        calibration_str_claim = f"{claim.calibration_score:.2f}" if claim.calibration_score is not None else "N/A"
        ...
        lines.extend([
            ...
            f"**Calibration Score:** {calibration_str_claim}",
...
def main():
    ...
    # Generate markdown report to separate file (engine output, not evidence)
    markdown = render_markdown_report(report)
    output_file = "Z2_REVIEW_PR465_ENGINE_OUTPUT.md"
    with open(output_file, "w") as f:
        f.write(markdown)
    print(f"\n[Engine] Markdown report written to {output_file}", file=sys.stderr)
