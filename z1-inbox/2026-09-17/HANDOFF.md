...
3. Z2: rule d31 on the board (tap → merge); on `serve behind login`, the four dashboard steps in runbook §5 — including the policy receipt after the Access step and the third Worker variable `ACCESS_ALLOWED_EMAILS`, then the three probes (the third, authorization, is NO_GATE until a second identity can be tested). Then d20–d30 and d32 from the Decisions page, each a tap → PR → merge.
