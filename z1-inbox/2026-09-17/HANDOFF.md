...
until it rules): `board/worker.js`, a Cloudflare Worker that serves `ui/` only behind a verified Cloudflare Access
login (RS256 against the team's keys; issuer, audience, expiry, signature) and answers 401 to everything but
`/healthz` until `ACCESS_TEAM_DOMAIN`/`ACCESS_AUD` are set — 20-check self-test in `board/worker.test.mjs`;
`wrangler.jsonc`, the Workers Builds configuration (assets `./ui`, `run_worker_first`, `keep_vars`). No
