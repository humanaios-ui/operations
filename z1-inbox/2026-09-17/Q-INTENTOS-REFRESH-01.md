...
|---|---|---|
| `tools/intent_os_board_reseal_v1_0.py` | classifies the checker's drift: **MECHANICAL** (`z1-inbox/INDEX.yaml`, `REGISTERED.md`, `PRIORITY_QUEUE.md`, `ledgers/NF_LEDGER.jsonl`, `ledgers/RESOURCE_LEDGER.jsonl`, `Z1_INBOX_INDEX.md`, `TOOLS_MANIFEST.md`, `CONTROLLED_DOCUMENTS.md`) vs **NEEDS-HUMAN** (everything else, and any MISSING / NOT-IN-HISTORY row). `--apply` re-hashes only mechanical rows, rewrites each description to *"(re-sealed <date> at <head>; content changed, description not re-read)"*, moves `read.against` to HEAD (the old sha joins the prior-reads list), advances `rev` (ISO stamp, so an open browser copy loads the new data on restore and keeps its taps), and refuses outright if any NEEDS-HUMAN row exists. Exit 0 HOLDS · 1 mechanical · 2 human. | `--self-test` plants: clean → HOLDS; inbox+registry → MECHANICAL, applied, HOLDS, notes replaced not stacked, prior read kept, rev advanced; apply on HOLDS → no-op; a tool changed alongside the inbox → NEEDS-HUMAN, refused, board byte-identical; a mechanical path MISSING → NEEDS-HUMAN. Registered `HAIOS-TOOL-165`. Row `t0-board-reseal` in the harness. |
| `.github/workflows/intent-os-refresh.yml` | on push to `main` touching the sealed and governance paths, daily 05:41 UTC, and by hand: tool self-tests first (a job that cannot tell a plant from a pass does not judge); seal check; drift classification; harness `--render`; job summary; receipt + board report + rendered pages as a 30-day run artifact. **Gated on `vars.INTENT_OS_REFRESH_AUTOPR == 'true'`:** MECHANICAL → `--apply`, re-render, one PR `intent-os/refresh` (updated in place, `audit:auto`, `smag_p: 0.90`); NEEDS-HUMAN or a red harness row not in `INTENT_OS_KNOWN_RED` → one `intent-os-stale` issue (commented in place). It never signs, never touches `z1-inbox/`, never publishes the board (d17). | parses under `workflow-lint.yml`'s strict duplicate-key loader; `on: push · schedule · workflow_dispatch`; permissions `contents · pull-requests · issues: write` (same as `smag-consolidate.yml`) |
| the board, re-read at `e2b9a7a` | three mechanical rows re-hashed by the tool; the `ratify.py` row re-read by hand (description now says what 1.2.0 does and that Ruling 6 is pending); `read.against` → `e2b9a7a`; `rev` → `2026-09-17T00:28Z` | checker HOLDS at HEAD, 0 behind |
...
board: HOLDS · queue: 45 candidates · 38 awaiting Z2 · 38 past the 2-day window
```
