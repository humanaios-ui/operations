# Ruling request Q-BOARD-RULING-25 — board ruling d25
...
**Source:** `z1-inbox/2026-09-17/Q-INTENTOS-REFRESH-01.md` (the question as Z2 was asked it there) · `ui/intent-os-humanaios-v3_3.html` · `rulings[d25]` · filed under Z2 ruling d18: every board ruling lives in the same queue as every other Z2 decision, one block per decision
**Status:** AWAITING Z2 RATIFICATION
**Gates:** board pipeline step 22

## Question

Local copies of the board — coordinate by re-downloading `ui/intent-os-humanaios-v3_3.html` from the repository after each refresh (no new surface), or publish a rolling release asset `intent-os-latest` so a copy has one stable URL to refresh from?

## Context carried on the board

d17 ruled local only, no site/, no Pages. A release asset is repository-scoped (the same access as the file). d31 (a login-gated Worker) may moot this question: a served board has one URL and no local copy to refresh; if d31 rules serve behind login, `later` here is the honest answer.

## Options

- `re-download from the repository`
...
Event-based, no clock. If board pipeline step 22 — the step whose `blockedOn` names d25 — is marked
verified complete while this block is still `status: OPEN`, the work proceeded without the call it said it
needed: that step drops d25 from its `blockedOn`, and this block is withdrawn rather than carried.
Conversely, if this ruling lands and step 22 still cannot proceed, d25 was not the blocker: a successor
