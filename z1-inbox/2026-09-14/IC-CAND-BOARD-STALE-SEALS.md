...
name: "A governance surface pinned to commits a history reset removed — six days unnoticed"
status: CANDIDATE
class: IC
...
- **claim:** `ui/intent-os-humanaios-v3_3.html` carried three "verified records" naming commits
  ...
  (H-CAND batch append), plus a `read.against` of `5df54b7`. The repository history was reset at
  `21aec1e` on 2026-09-10 (57 commits total afterwards). From that moment none of the four commits
  was fetchable, yet the board — the Z2 decision surface — kept presenting them as verified until
  2026-09-14, when `tools/intent_os_board_check_v1_0.py` was written and returned STALE on first run.
  No RECEIPT-GAP callout was raised in the four sessions between (09-10 handoff, 09-11, 09-12, 09-13).
...
- **evidence_tier:** VERIFIED-LIVE — `git cat-file -t` on each of the four shas fails on main today;
...
- **falsifier:** if any of the four shas is reachable from `main` today (`git merge-base --is-ancestor
  <sha> origin/main` exits 0), the commits were not lost and this IC collapses to "the board named the
  wrong shas" — a lesser finding with the same mitigation. Row-level: if a RECEIPT-GAP for these seals
  exists in any handoff dated 2026-09-10 → 2026-09-13, the "six days unnoticed" claim is false and the
  block is withdrawn.
- **status:** OPEN.
...
Stated above per claim: any of the four shas reachable from `main`, or a prior RECEIPT-GAP for them in a
2026-09-10 → 09-13 handoff, falsifies the block as written.
