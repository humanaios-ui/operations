...
def analyze(rows: list[dict]) -> dict:
    ...
    eligible_rows = prediction_rows(rows)
    ...
    return {
        "n_rows": len(rows),
        "calibration_rows": len(eligible_rows),
        "void_rows": len(rows) - len(eligible_rows),
        "overall": {**overall, **rates(overall)},
        "by_substrate": {s: {**c, **rates(c)} for s, c in sorted(by_sub.items())},
        "trend": trend_by_date(eligible_rows),
        "acat_dimensions": map_dimensions(eligible_rows),
        "gap_drivers": gap_drivers(eligible_rows),
    }
...
def render_md(a: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"_Calibration rows: {a['calibration_rows']} · VOID rows excluded from rates: {a['void_rows']}._",
        "",
...
def smoke_test() -> int:
    ...
    a = analyze([
        {"substrate": "Copilot", "timestamp": "2026-07-08T00:00:00Z", "predicted": "smag_p:0.1", "measured": "merged=True; checks: success:1"},
        {"substrate": "Copilot", "timestamp": "2026-07-08T00:00:00Z", "predicted": "smag_p:0.2", "measured": "merged=True; checks: failure:3"},
        {"substrate": "human:x", "timestamp": "2026-07-09T00:00:00Z", "predicted": "smag_p:0.3", "measured": "merged=True; checks: in_progress:5"},
        {"substrate": "human:x", "timestamp": "2026-07-09T00:00:00Z", "predicted": "smag_p:0.4", "measured": "merged=False"},
        {"substrate": "human:y", "timestamp": "2026-07-10T00:00:00Z", "predicted": "VOID: missing smag_p", "measured": "merged=True; checks: failure:9"},
    ])
    o = a["overall"]
    ...
    assert a["n_rows"] == 5 and a["calibration_rows"] == 4 and a["void_rows"] == 1
