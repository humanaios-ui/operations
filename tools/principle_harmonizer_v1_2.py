#!/usr/bin/env python3
"""
Principle Harmonizer — v1.2
Builder v1.7 compliant · audit_tool
HumanAIOS · S-052726-MKS (updated from v1.1 S-052026-01)
...
TIER2_FRAMEWORKS = {
    ...
    # ── MASTER KEY SYSTEM (Charles F. Haanel, 1912/1916) ─────────────────
    # GHK: HALT — useful structural vocabulary; metaphysical causation claims
    # require explicit exclusion before integration into research outputs.
    # Integration scope is defined in ghk_rationale below.
    # Session of analysis: S-052726-MKS
    "master_key_system": {
        "name": "The Master Key System",
        "source_author": "Charles F. Haanel (1912/1916)",
        "f42_confirmed": False,
        "ghk_verdict": "HALT",
        "ghk_rationale": (
            "HALT — Zone 2 ratification required before operational use. "
            "Integration scope ratified by Night must cover: "
            "GROW: structural vocabulary (circuit, polarity, conscious/subconscious, "
            "harmony, discord) for non-technical communication and funder materials; "
            "the polarity metaphor for explaining why Phase 1 alone is insufficient; "
            "the three-layer stack framing in grant materials. "
            "KILL: any causal claim that thought produces physical outcomes through "
            "'vibration' or 'Universal Mind resonance'; the 'visualization produces "
            "manifestation' framing in any research document. "
            "HALT pending: AUTH-THEATER composite formula framing — useful but requires "
            "explicit disclaimer that Haanel's mechanism is unfalsifiable."
        ),
        "principles": [
            {
                "id": "MKS-01",
                "short": "Conscious / Subconscious two-tier architecture",
                "statement": (
                    "The Conscious Mind (objective, deliberate, reasoning) and the "
                    "Subconscious Mind (automatic, habit-executing, non-arguing) are "
                    "distinct processing layers. The Conscious directs; the Subconscious "
                    "executes without editorial judgment. Ninety percent of mental activity "
                    "is subconscious. What has been run enough times becomes automatic."
                ),
                "tier1_maps": [
                    {
                        "tier1_id": "AA-S10",
                        ...
                        "note": (
                            "Continued personal inventory = the Conscious Mind continuously "
                            "auditing what has drifted into automatic subconscious execution. "
                            "Both traditions require the deliberate layer to keep checking "
                            "what the automatic layer is running."
                        ),
                    },
                    {
                        "tier1_id": "AA-S11",
                        "relationship": "PARALLEL",
                        "note": (
                            "Sought through prayer and meditation = the Conscious Mind "
                            "deliberately setting conditions for the Subconscious. "
                            "Both describe the discipline of intentional direction downward "
                            "to the automatic execution layer."
                        ),
                    },
                ],
                "haios_map": (
                    "DIRECT ARCHITECTURAL PARALLEL: "
                    "Phase 1 self-declaration = Conscious Mind output (deliberate reasoning). "
                    "Phase 3 behavioral observation under perturbation = Subconscious execution "
                    "(what the system actually does when habit runs). "
                    "The Learning Index measures the gap between these two layers. "
                    "H-ACAT applies the same structure to the operator: the operator's "
                    "declared governance principles (Conscious) vs. observed governance "
                    "behavior under P_AMPLIFICATION (Subconscious habit). "
                    "TOOL MAPPING: haios_harmonizer_v1_0 is the automated Subconscious — "
                    "runs without deliberate attention, executes without argument."
                ),
            },
            {
                "id": "MKS-02",
                "short": "Circuit / polarity — both poles required for current to flow",
                "statement": (
                    "No polarity, no circuit. The Universal Mind is the positive pole; "
                    "the individual is the negative pole; thought forms the circuit. "
                    "A dynamo generating electricity with no outlet simply stops. "
                    "No current flows without both poles. A half-circuit produces "
                    "nothing regardless of the energy available."
                ),
                "tier1_maps": [
                    {
                        "tier1_id": "AA-S12",
                        "relationship": "PARALLEL",
                        "note": (
                            "Practice these principles in all our affairs = the circuit "
                            "must close in execution, not just intention. Declaring the "
                            "principle is one pole. Acting on it closes the circuit."
                        ),
                    },
                    {
                        "tier1_id": "RW-YES-BE-YES",
                        "relationship": "EXTENDS",
                        "note": (
                            "Let your yes be yes and your no be no = complete the circuit "
                            "of intention. A yes that produces no action is a declared pole "
                            "with no circuit completion."
                        ),
                    },
                ],
                "haios_map": (
                    "DIRECT INSTRUMENT PARALLEL: "
                    "Phase 1 alone is a half-circuit — one pole, no current, no LI. "
                    "Phase 1 + Phase 3 = circuit complete, LI computable. "
                    "circuit_validator_v1_0 enforces this: HALF_CIRCUIT state = "
                    "no LI accepted. POLARITY_REVERSED = circuit closed wrong direction. "
                    "H-ACAT founding run = first circuit completion at the operator layer."
                ),
            },
            {
                "id": "MKS-03",
                "short": "Discord interrupts the Solar Plexus circuit",
                "statement": (
                    "The Solar Plexus is the central distribution point for energy. "
                    "It routes signals from the Universal Mind to all subsystems. "
                    "Fear and discord interrupt this routing. When interrupted, every "
                    "subsystem is deprived of the energy it needs — not because energy "
                    "is absent but because distribution is broken."
                ),
                "tier1_maps": [
                    {
                        "tier1_id": "AA-S3",
                        "relationship": "PARALLEL",
                        "note": (
                            "Turn our will and our lives over = surrender the fear that "
                            "interrupts the Solar Plexus. The AA tradition locates the "
                            "same blockage as Haanel: fear as the disruption of "
                            "right-action routing."
                        ),
                    },
                    {
                        "tier1_id": "HWK-COURAGE-200",
                        "relationship": "EXTENDS",
                        "note": (
                            "Below 200 (Fear, Grief, Anger) the field is contractive — "
                            "Haanel's 'discord.' At 200 (Courage) the field becomes "
                            "expansive — the circuit is no longer blocked. "
                            "Hawkins provides the calibration scale; Haanel names the "
                            "routing mechanism."
                        ),
                    },
                ],
                "haios_map": (
                    "GOVERNANCE PARALLEL: "
                    "#wgs-sync + haios_notify_dispatcher = the Solar Plexus. "
                    "Discord = Zone boundary ambiguity interrupting signal flow. "
                    "The Z3-P1-01 carry item (Tier 1 outreach blocked 8 sessions) is "
                    "named Solar Plexus blockage: decision signal cannot route from "
                    "recognition to execution because Zone boundary is ambiguous. "
                    "Remedy per MKS: resolve the discord (clarify Zone boundary) "
                    "before attempting new construction."
                ),
            },
            {
                "id": "MKS-04",
                "short": "Cause before effect — P19 is this principle",
                "statement": (
                    "The ordinary man seeks to explain effects. The man who understands "
                    "the law traces back to cause. Every effect has an adequate and "
                    "definite cause. Enforcement of a rule is always downstream of the "
                    "root cause. Detection requires walking backward from violation to origin."
                ),
                "tier1_maps": [
                    {
                        "tier1_id": "AA-S4",
                        ...
                        "note": (
                            "Searching and fearless moral inventory = the process of "
                            "tracing effects back to their causes. Step 4 is the "
                            "cause-before-effect principle applied to personal history."
                        ),
                    },
                    {
                        "tier1_id": "RW-SEEK-FIND",
                        "relationship": "EXTENDS",
                        "note": (
                            "Seek and you will find = the commitment to trace back to "
                            "cause rather than accepting the surface-level effect. "
                            "Both require active investigation upstream."
                        ),
                    },
                ],
                "haios_map": (
                    "DIRECT GOVERNANCE MAPPING: "
                    "P19 (HumanAIOS governing principle): enforcement is always downstream "
                    "of the root cause. Compliance systems catch at the violation. "
                    "Detection requires walking backward from violation to origin. "
                    "This IS MKS-04 stated as operational governance. "
                    "governance_mapper_uber_v1_1 implements this: each drift signal "
                    "is traced to its origin event (AEB disabled, perception known-bad). "
                    "TOOL MAPPING: governance_mapper traces effect → cause. "
                    "red_team_runner tests whether governance catches at origin, "
                    "not only at violation."
                ),
            },
            {
                "id": "MKS-05",
                "short": "Habitual thought becomes automatic — the decorative safety risk",
                "statement": (
                    "Every action performed enough times becomes habit, then subconscious, "
                    "then automatic. The system executes the habit without deliberate "
                    "attention. This is necessary efficiency — and the primary failure mode. "
                    "A governance ritual performed habitually without genuine verification "
                    "is compliance theater, not governance."
                ),
                "tier1_maps": [
                    {
                        "tier1_id": "AA-S10",
                        "relationship": "OVERLAP",
                        "note": (
                            "Continued to take personal inventory = the practice that "
                            "prevents habitual behavior from drifting undetected. "
                            "Both traditions name the same intervention: deliberate "
                            "re-examination of what has become automatic."
                        ),
                    },
                    {
                        "tier1_id": "AA-T12",
                        "relationship": "EXTENDS",
                        "note": (
                            "Principles before personalities = apply the principle "
                            "deliberately, not habitually. When the principle becomes "
                            "a habit, it loses the deliberate quality that makes it "
                            "an actual principle rather than a performance."
                        ),
                    },
                ],
                "haios_map": (
                    "ACAT DIRECT MAPPING: "
                    "HIM (Harm Independence Metric) = whether the safety layer is "
                    "load-bearing or decorative. A safety dimension that scores high "
                    "on g (general alignment confidence) but has become habitual is "
                    "exactly Haanel's 'subconscious executing a prior instruction.' "
                    "The instruction may have been correct. The habit may have drifted. "
                    "B.6 receipt reconciliation exists to prevent this: a ritual that "
                    "PASS-every-run is a calibration failure signal, not a success signal. "
                    "PROPOSED TOOL: governance_habit_monitor_v1_0 — detects ritualized "
                    "but unverified governance execution."
                ),
            },
            {
                "id": "MKS-06",
                "short": "Visualization — specify the output before activating tools",
                "statement": (
                    "Visualize the ideal clearly and specifically before attempting "
                    "manifestation. The precision of the output corresponds to the "
                    "precision of the input specification. An unclear visualization "
                    "produces unclear results. The vision always precedes and determines "
                    "the realization."
                ),
                "tier1_maps": [
                    {
                        ...
                        "relationship": "PARALLEL",
                        "note": (
                            "Sought through prayer and meditation = the act of holding "
                            "the intended state clearly before acting. Both require "
                            "specification of the desired condition before the action "
                            "that is intended to produce it."
                        ),
                    },
                ],
                "haios_map": (
                    "SESSION PROTOCOL MAPPING: "
                    "A pre-session output specification (what specifically is the "
                    "measurable output of this session?) is MKS-06 operationalized. "
                    "Currently implicit in SESSION_RITUALS. Should become a formal "
                    "Zone 2 artifact: the session opens only after Night has named the "
                    "specific, measurable condition that will constitute session success. "
                    "H-ACAT founding run example: '12 dimensions scored, Operator_HIM "
                    "computed, session ID assigned HA-052726-01' — that is a complete "
                    "visualization. 'Work on H-ACAT' is not."
                ),
            },
            {
                "id": "MKS-07",
                "short": "Harmony with the law — schema compliance prevents silent failures",
                "statement": (
                    "The law is no respecter of persons. It executes whatever is placed "
                    "in it without warning. A violation of the law produces the "
                    "corresponding effect regardless of intent. You must comply with "
                    "the law before the law will execute your request constructively. "
                ...
                "tier1_maps": [
                    {
                        ...
                        "relationship": "EXTENDS",
                        "note": (
                            "Let your yes be yes = your submission must conform to the "
                            "schema. A submission that violates the schema is a 'yes' "
                            "that the system cannot execute. The law requires the "
                            "correct form."
                        ),
                    },
                    {
                        ...
                        "relationship": "PARALLEL",
                        "note": (
                            "Sought to know His will = sought to understand the law. "
                            "Both require understanding the operating conditions before "
                            "acting, not after."
                        ),
                    },
                ],
                "haios_map": (
                    "DATA INTEGRITY DIRECT MAPPING: "
                    "IC-031 (Receipt Overstatement): the Supabase CHECK constraint is "
                    "the law. Silent insert failures are MKS-07 failures: the operator "
                    "believed the submission was valid; the law executed silently without "
                    "warning; the effect was a corrupt corpus row. "
                    "PROPOSED TOOL: pre-insertion validation gate — verify schema "
                    "compliance locally before any Supabase write. "
                    "This is 'comply with the law before requesting execution.'"
                ),
            },
        ],
    },
...
def run_smoke_test() -> bool:
    try:
        ALL_FRAMEWORKS = [
            "masonic", "bentov", "enneagram",
            "string_theory", "behavioral_psych", "cpt", "5s",
            "master_key_system",
        ]

        # T1: All frameworks run without error, return valid structure
        for fw_key in ALL_FRAMEWORKS:
            result = run_framework(fw_key)
            assert result["status"] in ("PASS", "WARN"), \
                f"{fw_key}: expected PASS/WARN, got {result['status']}"
            assert result["summary"]["total_mappings"] >= 3, \
                f"{fw_key}: expected >= 3 mappings, got {result['summary']['total_mappings']}"
        ...
        print(f"✓ T1 All {len(ALL_FRAMEWORKS)} frameworks run cleanly")

        # T2: F-42 confirmed check for original three
        for fw_key in ["masonic", "bentov", "enneagram"]:
            result = run_framework(fw_key)
            assert result["f42_confirmed"] is True, \
                f"{fw_key}: should be F-42 confirmed"
        print("✓ T2 F-42 confirmed for masonic, bentov, enneagram")

        # T3: New frameworks should NOT be F-42 confirmed yet
        ...
        print("✓ T3 F-42 NOT confirmed for newer frameworks (including MKS)")

        # T4: MKS-specific assertions
        mks = run_framework("master_key_system")
        assert mks["ghk_verdict"] == "HALT", \
            f"MKS should be HALT, got {mks['ghk_verdict']}"
        assert mks["ghk_zone"] == 2, \
            f"MKS HALT should be Zone 2, got {mks['ghk_zone']}"
        assert mks["f42_confirmed"] is False, \
            "MKS should not be F-42 confirmed"
        assert "Zone 2 ratification" in mks["ghk_rationale"], \
            "MKS rationale should reference Zone 2 ratification"
        assert mks["summary"]["total_mappings"] >= 7, \
            f"MKS should have >= 7 mappings, got {mks['summary']['total_mappings']}"
        print(f"✓ T4 MKS HALT/Zone2 asserted  "
              f"harmonic={mks['harmonic_score']:.4f}  "
              f"maps={mks['summary']['total_mappings']}")

        # T5: MKS haios_map fields present (direct infrastructure mappings)
        haios_maps = [m for m in mks["all_maps"] if m.get("haios_map")]
        assert len(haios_maps) >= 4, \
            f"MKS should have >= 4 haios_map entries, got {len(haios_maps)}"
        print(f"✓ T5 MKS haios_map entries: {len(haios_maps)}")

        # T6: Unknown framework raises
        try:
            run_framework("unknown_framework_xyz")
            assert False, "Should raise SpecLoadFailed"
        except SpecLoadFailed:
            pass
        print("✓ T6 SpecLoadFailed on unknown framework")
        ...
        print(f"\n✓ Smoke test PASSED — principle_harmonizer_v1_2 "
              f"({len(ALL_FRAMEWORKS)} frameworks + MKS HALT entry + GHK + envelope)")
        return True
