#!/usr/bin/env python3
"""
ACAT Document Analyzer — v1.1
Builder v1.7 compliant · diagnostic_tool
HumanAIOS · S-051626-01-acat-tools-alternate-functions-mapping

Changes from v1.0:
- Keyword-vector scoring (TF-IDF surrogate): for each dimension, counts
  high-signal and low-signal term frequencies, producing a normalized
  evidence_density score (0-100) to complement interactive scoring.
  Useful for quick objective anchoring before human scoring.
- Batch mode (--batch-dir): score all .txt files in a directory and
  produce a ranked comparison CSV — identifies weakest governance docs.
- F-34 architectural detection extended to all 12 dimensions (was 4).
- Aggregator includes evidence_density_scores field for auditability.
...
def main():
    ...
    ed = compute_evidence_density(text) if text else {}
