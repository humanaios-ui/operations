# SKILL: haios_doc_ingestor_v1_0

## 1. Description

**What does this tool do?**  
Document ingestion pipeline for HumanAIOS surfaces.
...
This tool exists to support the `infrastructure_tool` workflow in this repository and provide repeatable command-line execution for the `haios_doc_ingestor_v1_0` process.
...
| `--session` | `string` | No | `DEFAULT_CONFIG["session_id"]` | Command-line option for this tool. |
| `--dry-run` | `boolean` | No | `false` | Command-line option for this tool. |
| `--regenerate-state-only` | `boolean` | No | `false` | Command-line option for this tool. |
| `--smoke-test` | `boolean` | No | `false` | Command-line option for this tool. |
| `--config` | `string` | No | `None` | Command-line option for this tool. |
...
python tools/haios_doc_ingestor_v1_0.py --help
```
