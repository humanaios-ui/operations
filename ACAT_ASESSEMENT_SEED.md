...
1. **Variant registry** — enumerates the full family of ACAT instruments (standard, T2, Shadow, h-ACAT, hCAT, org-ACAT, Enterprise SOP) with their scope, status, and relationship to the canonical corpus.
1. **API/tool alignment** — specifies the exact parameter contract the assessment endpoints implement, so any tool, wrapper, MCP server, or Apps Script that submits data stays in schema.
...
1. **Monitoring cycle** — regular re-runs on cadence (weekly, monthly)
1. **Anomaly detection** — flag dimensional drift, LI drift, or new behavioral flags
1. **Reporting** — structured output for human review
...
1. **Observer effect** — cannot distinguish genuine updating from performance for the instrument
1. **Policy attractors** — frozen policy systems will score identically regardless of calibration data; the instrument detects this but cannot correct for it
1. **Single-session validity** — one session is insufficient to characterize a system; LI trajectories across sessions are more meaningful than single-session LI
